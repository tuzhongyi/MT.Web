    ...
    FaceSet: {
        List: {
            create: function (listId, deviceId) {
                Property.FaceSet.value = new Dictionary();
                Property.FaceSet.list(deviceId);
                var nvrs = Property.FaceSet.value.toArray();
                for (var a = 0; a < nvrs.length; a++) {
                    ...
                    box.className += "del set col-xlg-2 col-lg-3 col-md-4 col-sm-6 group-item";
                    box.id = nvrs[a].Id;
                    ...
                    card.Text.setSubText(Language.Enum.FaceSetType[nvrs[a].Type]);
                    card.DeleteButton.setClick(function (e) {
                        var id = e.currentTarget.elementId;
                        $.confirm({
                            text: chinese.delete_prompt + chinese.face_database,
                            okButton: chinese.ok,
                            ...
                            top: -1,
                            confirm: function () {
                                Html.FaceSet.remove(Html.DeviceId, id);
                            }
                        });
                        stopPropagation();
                    });
                    card.SetButton.setClick(function (e) {
                        var id = e.currentTarget.elementId;
                        jumpHref.href = navigation.device.nvr.faceset.details + "?deviceId=" + Html.DeviceId + "&faceSetId=" + id + "&type=" + Html.Type;
                        loadContent('page', base64encode(setUrlRandomParams(navigation.device.nvr.faceset.details + "?deviceId=" + Html.DeviceId + "&faceSetId=" + id + "&type=" + Html.Type)));
                        stopPropagation();
                    });
        ...
        remove: function (deviceId, faceSetId) {
            var result = Property.FaceSet.remove(deviceId, faceSetId);
            if (result) {
                delete Property.FaceSet.value[faceSetId];
                var div = document.getElementById(faceSetId);
        ...
        add: function () {
            jumpHref.href = navigation.device.nvr.faceset.details + "?deviceId=" + Html.DeviceId + "&type=" + Html.Type;
            loadContent('page', base64encode(setUrlRandomParams(navigation.device.nvr.faceset.details + "?deviceId=" + Html.DeviceId + "&type=" + Html.Type)));
        }
    }
}
    ...
    FaceSet: {
        ...
        list: function (deviceId) {
            ...
            if (result && result.FaceSet) {
                for (var i = 0; i < result.FaceSet.length; i++) {
                    ...
                    if (!this.value[result.FaceSet[i].Id]["Selected"])
                        this.value[result.FaceSet[i].Id]["Selected"] = false;
                }
            }
            return result;
        },
        remove: function (deviceId, faceSetId) {
            return tryCatch(function () {
                return Client.Management().Device.FaceSets.Delete(deviceId, faceSetId);
            });
        },
