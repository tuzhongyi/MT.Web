...
//Association
//输入通道选择事件
function DepartmentAssociationIOInput_Selected(sender, args) {
    tryCatch(function () {
...
//删除单个输入通道
function GroupListItem_DepartmentAssociationRemoveIOInput_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
function DepartmentAssociationIOInput_PageChange(index) {
    ioinputList.Association.clear();
...
function DepartmentAssociationIOInput_SelectedAll(items) {
...
function DepartmentAssociationIOInput_SelectedCancel(items) {
    ioinputList.Association.selected = new Array();
}
function DepartmentAssociationIOInput_SelectedInverse() {
    ioinputList.Association.selectInverse();
}
function DepartmentAssociationIOInput_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.IOInput.Association.list);
        if (ioinputList.Association.selected.indexOf(id) < 0)
            ioinputList.Association.selected.push(id);
...
function GroupListItem_Department_IOInput_Detail_Click(sender, args) {
    stopPropagation();
...
//未绑定输入通道绑定事件
function GroupListItem_DepartmentUnassociationAddIOInput_Click(sender, args) {
    stopPropagation();
    AlertWindow.Show(sender, -1);
    return false;
}
//未绑定输入通道选择事件
function DepartmentUnassociationIOInput_Selected(sender, args) {
    tryCatch(function () {
...
function DepartmentUnassociationIOInput_PageChange(index) {
    ioinputList.Unassociation.clear();
    ioinputList.Unassociation.load(index);
}
function DepartmentUnassociationIOInput_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
...
function DepartmentUnassociationIOInput_SelectedCancel(items) {
    ioinputList.Unassociation.selected = new Array();
}
function DepartmentUnassociationIOInput_SelectedInverse() {
    ioinputList.Unassociation.selectInverse();
}
function DepartmentUnassociationIOInput_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.IOInput.Unassociation.list);
        if (ioinputList.Unassociation.selected.indexOf(id) < 0)
            ioinputList.Unassociation.selected.push(id);
