...
//用户选择事件
function DepartmentAssociationUser_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.User.Association.list);
...
//删除单个用户
function GroupListItem_DepartmentAssociationRemoveUser_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.User.Association.remove);
        $.confirm({
            text: "确认要删除这个用户吗？",
            okButton: "确定",
            cancelButton: "取消",
...
//已绑定用户页面改变事件
function DepartmentAssociationUser_PageChange(index) {
    userList.Association.clear();
...
//已绑定的用户当前页全部选中事件
function DepartmentAssociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.User.Association.list);
...
//已绑定的用户当前页全部选择取消事件
function DepartmentAssociationUser_SelectedCancel(items) {
    userList.Association.selected = new Array();
}

function DepartmentAssociationUser_SelectedInverse() {
    userList.Association.selectInverse();
}

function DepartmentAssociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.User.Association.list);
        if (userList.Association.selected.indexOf(id) < 0)
            userList.Association.selected.push(id);
...
//未绑定用户绑定事件
function GroupListItem_DepartmentUnassociationAddUser_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
//未绑定用户选择事件
function DepartmentUnassociationUser_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.User.Unassociation.list);
        userList.Unassociation.select(id);
    });
}

function DepartmentUnassociationUser_PageChange(index) {
    userList.Unassociation.clear();
    userList.Unassociation.load(index);
}
function DepartmentUnassociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
    }
    userList.Unassociation.selectAll();
}
function DepartmentUnassociationUser_SelectedCancel(items) {
    userList.Unassociation.selected = new Array();
}

function DepartmentUnassociationUser_SelectedInverse() {
    userList.Unassociation.selectInverse();
}

function DepartmentUnassociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
