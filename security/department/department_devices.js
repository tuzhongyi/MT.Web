...
//Association
//设备选择事件
function DepartmentAssociationDevice_Selected(sender, args) {
    tryCatch(function () {
...
//删除单个设备
function GroupListItem_DepartmentAssociationRemoveDevice_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
function DepartmentAssociationDevice_PageChange(index) {
    deviceList.Association.clear();
...
function DepartmentAssociationDevice_SelectedAll(items) {
...
function DepartmentAssociationDevice_SelectedCancel(items) {
    deviceList.Association.selected = new Array();
}
function DepartmentAssociationDevice_SelectedInverse() {
    deviceList.Association.selectInverse();
}

function DepartmentAssociationDevice_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.Association.list);
        if (deviceList.Association.selected.indexOf(id) < 0)
            deviceList.Association.selected.push(id);
...
function GroupListItem_Department_Device_Detail_Click(sender, args) {
    stopPropagation();
...
//未绑定设备绑定事件
function GroupListItem_DepartmentUnassociationAddDevice_Click(sender, args) {
    stopPropagation();
    AlertWindow.Show(sender, -1);
    return false;
}
//未绑定设备选择事件
function DepartmentUnassociationDevice_Selected(sender, args) {
    tryCatch(function () {
...
function DepartmentUnassociationDevice_PageChange(index) {
    deviceList.Unassociation.clear();
    deviceList.Unassociation.load(index);
}
function DepartmentUnassociationDevice_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
...
function DepartmentUnassociationDevice_SelectedCancel(items) {
    deviceList.Unassociation.selected = new Array();
}

function DepartmentUnassociationDevice_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.Unassociation.list);
        if (deviceList.Unassociation.selected.indexOf(id) < 0)
            deviceList.Unassociation.selected.push(id);
...
function DepartmentUnassociationDevice_SelectedInverse() {
