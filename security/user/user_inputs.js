...
//Association
//输入通道选择事件
function UserAssociationInput_Selected(sender, args) {
    tryCatch(function () {
...
//删除单个输入通道
function GroupListItem_UserAssociationRemoveInput_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
function UserAssociationInput_PageChange(index) {
    inputList.Association.clear();
...
function UserAssociationInput_SelectedAll(items) {
...
function UserAssociationInput_SelectedCancel(items) {
    inputList.Association.selected = new Array();
}

function UserAssociationInput_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Input.Association.list);
        if (inputList.Association.selected.indexOf(id) < 0)
            inputList.Association.selected.push(id);
...
function UserAssociationInput_SelectedInverse() {
...
function GroupListItem_User_Input_Detail_Click(sender, args) {
    stopPropagation();
...
//未绑定输入通道绑定事件
function GroupListItem_UserUnassociationAddInput_Click(sender, args) {
    stopPropagation();
    AlertWindow.Show(sender, -1);
    return false;
}
//未绑定输入通道选择事件
function UserUnassociationInput_Selected(sender, args) {
    tryCatch(function () {
...
function UserUnassociationInput_PageChange(index) {
    inputList.Unassociation.clear();
    inputList.Unassociation.load(index);
}
function UserUnassociationInput_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
...
function UserUnassociationInput_SelectedCancel(items) {
    inputList.Unassociation.selected = new Array();
}
function UserUnassociationInput_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Input.Unassociation.list);
        if (inputList.Unassociation.selected.indexOf(id) < 0)
            inputList.Unassociation.selected.push(id);
...
function UserUnassociationInput_SelectedInverse() {
