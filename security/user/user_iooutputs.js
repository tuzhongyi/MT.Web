...
//Association
//输出通道选择事件
function UserAssociationIOOutput_Selected(sender, args) {
    tryCatch(function () {
...
//删除单个输出通道
function GroupListItem_UserAssociationRemoveIOOutput_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
function UserAssociationIOOutput_PageChange(index) {
    iooutputList.Association.clear();
...
function UserAssociationIOOutput_SelectedAll(items) {
...
function UserAssociationIOOutput_SelectedCancel(items) {
    iooutputList.Association.selected = new Array();
}
function UserAssociationIOOutput_SelectedInverse() {
    iooutputList.Association.selectInverse();
}
function UserAssociationIOOutput_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.IOOutput.Association.list);
        if (iooutputList.Association.selected.indexOf(id) < 0)
            iooutputList.Association.selected.push(id);
...
function GroupListItem_User_IOOutput_Detail_Click(sender, args) {
    stopPropagation();
...
function btn_iooutput_association_Click(sender, args) {
    if (iooutputList.Unassociation.selected.length > 0)
        PageEvent.IOOutput.BatchAddIOOutputsToUser(IOOutputPermissions.All);
    else
...
//未绑定输出通道绑定事件
function GroupListItem_UserUnassociationAddIOOutput_Click(sender, args) {
    stopPropagation();
    //AlertWindow.Show(sender, -1);
    PageEvent.IOOutput.AddIOOutputToUser(IOOutputPermissions.All, args);
    return false;
}
//未绑定输出通道选择事件
function UserUnassociationIOOutput_Selected(sender, args) {
    tryCatch(function () {
...
function UserUnassociationIOOutput_PageChange(index) {
    iooutputList.Unassociation.clear();
    iooutputList.Unassociation.load(index);
}
function UserUnassociationIOOutput_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
...
function UserUnassociationIOOutput_SelectedCancel(items) {
    iooutputList.Unassociation.selected = new Array();
}
function UserUnassociationIOOutput_SelectedInverse() {
    iooutputList.Unassociation.selectInverse();
}
function UserUnassociationIOOutput_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.IOOutput.Unassociation.list);
        if (iooutputList.Unassociation.selected.indexOf(id) < 0)
            iooutputList.Unassociation.selected.push(id);
