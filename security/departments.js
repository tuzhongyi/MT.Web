...
function security_department_list_load() {
    var li = new Object();
    ...
    var win = new AlertWindow("", "<a class='btn btn-xs btn-primary-outline pull-right div-xsgroup' style='font-size:14px;padding-left:14px;'><i class='icon-plus'></i></a>", "security/department/department_details.htm", -1);
    document.getElementById("btn-addDepartment").appendChild(win);
...
function GroupListItem_SelectAll(args) {
    ...
    for (var i = 0; i < args.length; i++) {
        ...
        Property.DepartmentList.value[id].Selected = true;
    }
    Html.Control.GroupList.selectedCount = args.length;
};
function GroupListItem_Cancel() {
    for (var key in Property.DepartmentList.value) {
        Property.DepartmentList.value[key].Selected = false;
    }
    Html.Control.GroupList.selectedCount = 0;
};
function removeDepartment_Click(sender, args) {
    var id = args;
    stopPropagation();
    $.confirm({
        text: "确定要删除这个部门吗？",
...
function batchRemoveDepartments_Click(sender, args) {
...
function editDepartment_Click(sender, args) {
    stopPropagation();
