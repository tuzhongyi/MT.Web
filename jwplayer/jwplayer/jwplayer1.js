"undefined" == typeof jwplayer && (jwplayer = function (d) {
    ...
    function (d) {
        ...
        function k(b) {
            ...
            return function () {
                ...
                (c = b.parseXML(m.responseText)) && c.firstChild ? (m = b.extend({}, m, { responseXML: c }),
        ...
        b.exists = function (b) {
            switch (typeof b) {
                case "string": return 0 < b.length;
                case "object": return null !== b;
                case "undefined":
                    return !1
            }
            return !0
        ...
        b.isIPad = a(/iPad/i);
        b.isSafari602 = a(/Macintosh.*Mac OS X 10_8.*6\.0\.\d* Safari/i); b.isSafari = function () {
        ...
        b.deepReplaceKeyName = function (a, c, e) {
            switch (d.utils.typeOf(a)) {
                case "array": for (var g = 0; g < a.length; g++) a[g] = d.utils.deepReplaceKeyName(a[g], c, e);
                    break;
                case "object": b.foreach(a, function (b, g) {
                    var j;
        ...
        b.foreach = function (a, c) {
            var e, g;
            for (e in a)
                "function" == b.typeOf(a.hasOwnProperty) ? a.hasOwnProperty(e) && (g = a[e], c(e, g)) : (g = a[e], c(e, g))
        };
        b.isHTTPS = function () {
            ...
            if (h && b.exists(e.XDomainRequest))
                ...
                g.onreadystatechange = function () {
                    ...
                    if (4 === d.readyState) switch (d.status) {
                        case 200: f(d, t, c, j)(); break;
                        case 404: j("File not found")
                    }
                };
                g.onerror = k(j, a)
            }
                ...
                for (g = 0; g < a.length; g++)
                    if (j = b.extend({}, a[g]), j.sources = b.filterSources(j.sources, !0), 0 < j.sources.length) {
                        for (f = 0; f < j.sources.length; f++)
                            h = j.sources[f], h.label || (h.label = f.toString()); e.push(j)
                    }
            return e
        };
    ...
    function (d) {
        ...
        d.touch = function (f) {
            ...
            function e(a, b, c) {
                if (n[a] && (b.preventManipulation && b.preventManipulation(), b.preventDefault && b.preventDefault(), b = c ? c : j(a, b)))
                    n[a](b)
            }
            function j(a, c) {
                var e = null;
                ...
                var j = b.getBoundingClientRect(),
                    e = {
                        type: a, target: b, x: e.pageX - window.pageXOffset - j.left, y: e.pageY, deltaX: 0, deltaY: 0
                    };
    ...
    function (d) {
        ...
        a.encrypt = function (h, e) {
            ...
            for (var b = a.strToLongs(f.encode(e).slice(0, 16)), c = j.length, d = j[c - 1], m = j[0], p, l = Math.floor(6 + 52 / c), g = 0; 0 < l--;) {
                ...
                p = g >>> 2 & 3;
                for (var q = 0; q < c; q++)
                    m = j[(q + 1) % c], d = (d >>> 5 ^ m << 2) + (m >>> 3 ^ d << 4) ^ (g ^ m) + (b[q & 3 ^ p] ^ d), d = j[q] += d
            }
            j = a.longsToStr(j);
            return k.encode(j)
        };
        a.decrypt = function (h, e) {
            ...
            for (var j = a.strToLongs(k.decode(h)), b = a.strToLongs(f.encode(e).slice(0, 16)), c = j.length, d = j[c - 1], m = j[0], p, l = 2654435769 * Math.floor(6 + 52 / c); 0 != l;) {
                p = l >>> 2 & 3;
                for (var g = c - 1; 0 <= g; g--)
                    d = j[0 < g ? g - 1 : c - 1], d = (d >>> 5 ^ m << 2) + (m >>> 3 ^ d << 4) ^ (l ^ m) + (b[g & 3 ^ p] ^ d), m = j[g] -= d;
                l -= 2654435769
            }
            j = a.longsToStr(j);
            j = j.replace(/\0+$/, "");
        ...
        var k = {
            ...
            encode: function (a, e) {
                var j,
                    ...
                    g,
                    q = k.code;
                g = ("undefined" == typeof e ? 0 : e) ? f.encode(a) : a; l = g.length % 3;
                if (0 < l)
                    for (; 3 > l++;)
                        p += "\x3d", g += "\x00";
                for (l = 0; l < g.length; l += 3)
                    j = g.charCodeAt(l), b = g.charCodeAt(l + 1), c = g.charCodeAt(l + 2), d = j << 16 | b << 8 | c, j = d >> 18 & 63, b = d >> 12 & 63, c = d >> 6 & 63, d &= 63, m[l / 3] = q.charAt(j) + q.charAt(b) + q.charAt(c) + q.charAt(d);
                m = m.join(""); return m = m.slice(0, m.length - p.length) + p
            },
            decode: function (a, e) {
                ...
                for (var q = 0; q < l.length; q += 4)
                    j = g.indexOf(l.charAt(q)), b = g.indexOf(l.charAt(q + 1)), d = g.indexOf(l.charAt(q + 2)), m = g.indexOf(l.charAt(q + 3)), c = j << 18 | b << 12 | d << 6 | m, j = c >>> 16 & 255, b = c >>> 8 & 255, c &= 255, p[q / 4] = String.fromCharCode(j, b, c), 64 == m && (p[q / 4] = String.fromCharCode(j, b)), 64 == d && (p[q / 4] = String.fromCharCode(j)); d = p.join(""); return e ? f.decode(d) : d
            }
        }, f = {
            encode: function (a) {
                a = a.replace(/[\u0080-\u07ff]/g,
                ...
                if ("undefined" != a.typeOf(h[j]))
                    for (var c = 0; c < h[j].length; c++) {
                        try {
                            ...
                            a.log("There was an error while handling a listener: " + n.toString(),
                                h[j][c].listener)
                        }
    ...
    function (d) {
        ...
        d.plugins.pluginloader = function (h, e) {
            function j() {
                m ? g.sendEvent(k.ERROR, { message: p }) : n || (n = !0, c = a.loaderstatus.COMPLETE, g.sendEvent(k.COMPLETE))
            }
            function b() {
                l || j();
                if (!n && !m) {
                ...
                f(c.plugins, function (f, h) {
                    ...
                    try {
                        if (q && c.plugins && c.plugins[p]) {
                            ...
                            A.id = b.id + "_" + k; A.style.position = "absolute";
                            A.style.top = 0;
                            A.style.zIndex = j + 10;
        ...
        (a.jwparser = function () { }).parseEntry = function (k, f) {
            ...
            if (h.length) {
                ...
                for (b = 0; b < h.length; b++)
                    0 < h[b].file.length && (h[b]["default"] = "true" == h[b]["default"] ? !0 : !1, h[b].label.length || delete h[b].label, f.sources.push(h[b]))
            }
            if (e.length) {
                f.tracks = [];
                for (b = 0; b < e.length; b++)
                    0 < e[b].file.length && (e[b]["default"] = "true" == e[b]["default"] ? !0 : !1, e[b].kind = !e[b].kind.length ? "captions" : e[b].kind, e[b].label.length || delete e[b].label, f.tracks.push(e[b]))
            }
            return f
        }
    }(jwplayer),
                    ...
                    if (d = b.childNodes[m], "media" == d.prefix && f(d))
                        switch (f(d).toLowerCase()) {
                            case "content":
                                ...
                                k(d, "url") && (c.sources || (c.sources = []), c.sources.push({ file: k(d, "url"), type: k(d, "type"), width: k(d, "width"), label: k(d, "label") }));
                                break;
                            case "title":
                                c.title = h(d);
                                break;
    ...
    function (d) {
        var a = d.item = function (k) {
            ...
            for (var e = 0; e < h.sources.length; e++) {
                ...
                h.sources[e]["default"] = j ? "true" == j.toString() : !1; h.sources[e] = new d.source(h.sources[e])
            }
                        ...
                        if ("string" == a.typeOf(r.playlist)) {
                            ...
                            for (g = 0; g < r.modes.length; g++)
                                if (r.modes[g].type && j[r.modes[g].type]) {
                                    ...
                                    if (w.supportsConfig())
                                        return w.addEventListener(k.ERROR, m), w.embed(), c(b, u.events), b
                                }
                            if (r.fallback) {
                                ...
                                x = "No suitable players found and fallback disabled", q(x, h), a.log(x), t.parentNode.replaceChild(v, t)
                        }
                }
                function m(a) {
                    g(t, B + a.message)
                ...
                function g(b, c) {
                    if (r.fallback) {
                        ...
                        var e = document.createElement("p"),
                            d = e.style; d.verticalAlign = "middle";
                        d.textAlign = "center";
    ...
    function (d) {
        ...
        (d.embed.config = function (e) {
            var j = {
                ...
                primary: "html5",
                width: 480,
                base: e.base ? e.base : f.getScriptPath("jwplayer1.js"),
                aspectratio: ""
            };
        ...
        d.embed.download = function (d, h, e) {
            ...
            if (c && c.length) {
                ...
                l ? (p = l,
                    ...
                    q ? (h = q,
                        d = b("embed", "", d),
                        d.src = "http://www.youtube.com/v/" + a.youTubeID(h),
    ...
    function (d) {
        ...
        (d.embed.flash = function (e, j, b, c, n) {
            ...
            this.embed = function () {
                ...
                f[e.id] = v;
                a.isIE() ? (
                    h = '\x3cobject classid\x3d"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" " width\x3d"100%" height\x3d"100%"id\x3d"' + e.id + '" name\x3d"' + e.id + '" tabindex\x3d0""\x3e',
        ...
        var a = d.embed,
            ...
            f = k.extend(function (f) {
                ...
                switch (r) {
                    ...
                    case "pro": b.sharing && (b.sharing.client && b.sharing.client.match(".js$|.swf$") && (l = b.sharing.client), n[l] = b.sharing), b.skin && (c.skin = b.skin.replace(/^(beelden|bekle|five|glow|modieus|roundster|stormtrooper|vapor)$/i, k.repo() + "skins/$1.xml"))
                }
                c.plugins = n;
                return new a(f)
            }, a);
