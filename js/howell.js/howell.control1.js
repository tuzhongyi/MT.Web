    ...
    for (var item in Enum) {
        ...
        switch (model) {
            ...
            case ControlModel.EnumAndLanguage:
                option.innerHTML = item + " - " + language[item];
                break;
...
GroupList.Select = {
    ...
    Inverse: function (name, name_is_class) {
        var items = name_is_class ? document.getElementsByClassName(name) : getTag(name, getTagType.Name);
        for (var i = 0; i < items.length; i++) {
            items[i].click();
        }
...
AlertWindow.Show = function (trigger, top, fn) {
    /// <signature>
...
function SearchControl(id, fn, checkDiv) {
    ...
    btnContentDiv.className = "btn btn-tenderblue-outline";
...
function DropDownListWindow(dropDownListAttributes) {
    ...
    caretDiv.className = "btn btn-xs dropdown-toggle control btn-tenderblue-outline btn-xsgroup-toggle";
