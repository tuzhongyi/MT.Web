...
Property["IOInput"] = {
    load: function (userId,ioinputId) {
        return tryCatch(function () {
            return Property.IOInputList.Association.get(userId, ioinputId);
        });
    },
    modify: function (ioinput) {
        return tryCatch(function () {
            return Client.Security().User.IO.Input.Set(Html.Current.Id.get(), ioinput);
        });
    },
}
...
Html.Control.PopoverWindow.IOInputList.AlertWindow["Details"] = {
    load: function (userId, ioinputId) {
        var result = Property.IOInput.load(userId, ioinputId);
