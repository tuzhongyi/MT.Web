...
Property["Input"] = {
    load: function (departmentId,inputId) {
        return tryCatch(function () {
            return Property.InputList.Association.get(departmentId, inputId);
        });
    },
    modify: function (input) {
        return tryCatch(function () {
            return Client.Security().Department.Video.Input.Set(Html.Current.Id.get(), input);
        });
    },
}
...
Html.Control.PopoverWindow.InputList.AlertWindow["Details"] = {
    load: function (departmentId, inputId) {
        var result = Property.Input.load(departmentId, inputId);
