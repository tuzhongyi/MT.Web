...
Property["IOOutput"] = {
    load: function (departmentId, iooutputId) {
        return tryCatch(function () {
            return Property.IOOutputList.Association.get(departmentId, iooutputId);
        });
    },
    modify: function (iooutput) {
        return tryCatch(function () {
            return Client.Security().Department.IO.Output.Set(Html.Current.Id.get(), iooutput);
        });
    },
}
...
Html.Control.PopoverWindow.IOOutputList.AlertWindow["Details"] = {
    load: function (departmentId, iooutputId) {
        var result = Property.IOOutput.load(departmentId, iooutputId);
        if (result) {
