...
Property["DepartmentList"] =
{
    Association: {
        ...
        load: function (inputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.IO.Input.Department.List(inputId, index, size)
            });
        ...
        remove: function (inputId, departmentId) {
            var result = tryCatch(function () {
                return Client.Security().Permission.IO.Input.Department.Delete(inputId, departmentId);
            })
            if (result)
    ...
    Unassociation: {
        ...
        load: function (inputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.IO.Input.Department.List(inputId, index, size, true);
            });
            var result = new Object();
        ...
        add: function (inputId, departmentId, permission) {
            var input = Client.Management().Device.IO.Input.Get(Property.DeviceId, inputId);
            var ioinputChannelPermission = new IOInputChannelPermission();
            ioinputChannelPermission.Id = input.Id;
            ioinputChannelPermission.Name = input.Name;
            ioinputChannelPermission.Permission = permission;
            ioinputChannelPermission.IOInputChannel = input;
            Client.Security().Department.IO.Input.Create(departmentId, ioinputChannelPermission);
            delete this.value[departmentId];
...
Html.Control.PopoverWindow["DepartmentList"] = {
    Common: {
        createItem: function (key, department) {
            var createCoontrol = {
                ...
                UnassociationOperationButton: function (department) {
                    ...
                    a.href = Property.Location.Department.Details.Value + "?fun=PageEvent.Device.IO.Input.Department.AddDepartmentToInput&inputId=" + Html.Control.AlertWindow.Device.Input.Details.get().Id + "&departmentId=" + department.Id
                    a.setAttribute("onclick", "return GroupListItem_IOInputUnassociationAddDepartment_Click(this, '" + department.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.IO.Input.Department.Unassociation.add + department.Id;
                    a.title = "添加";
                    return a;
                },
                AssociationOperationButton: function (department) {
                    ...
                    a.setAttribute("onclick", "GroupListItem_IOInputAssociationRemoveDepartment_Click(this, '" + department.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.IO.Input.Department.Association.remove + department.Id;
                    a.title = "删除";
                    return a;
                },
            ...
            item = new GroupListItem(Info.ControlIdPrefix.Device.IO.Input.Department[key].list, eval("IOInput" + key + "Department_Selected"));
            item.id = Info.ControlIdPrefix.Device.IO.Input.Department[key].list + department.Id;
        ...
        load: function (key, index) {
            ...
            if (items) {
                ...
                pagination = new Pagination("", page.PageIndex, page.PageSize, page.PageCount, page.RecordCount, page.TotalRecordCount, eval("IOInput" + key + "Department_PageChange"));
    ...
    Association: {
        ...
        selectInverse: function () {
            Property.DepartmentList.Association.load(Html.Control.AlertWindow.Device.Input.Details.get().Id);
            for (var uid in Property.DepartmentList.Association.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.IO.Input.Department.Association.list + uid);
        ...
        load: function (index) {
            ...
            PageEvent.Device.IO.Input.GroupListDepartmentIconButtonChanged(Html.Control.AlertWindow.Device.Input.Details.get().Id, Property.DepartmentList.Association.page.TotalRecordCount);
        },
        clear: function () {
    ...
    Unassociation: {
        ...
        selectInverse: function () {
            Property.DepartmentList.Unassociation.load(Html.Control.AlertWindow.Device.Input.Details.get().Id);
            for (var uid in Property.DepartmentList.Unassociation.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.IO.Input.Department.Unassociation.list + uid);
...
PageEvent.Device.IO.Input.Department.AddDepartmentToInput = function (permission, departmentId) {
...
PageEvent.Device.IO.Input.Department.BatchAddInputsToDepartment = function (permission) {
