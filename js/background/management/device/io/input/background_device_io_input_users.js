...
Property["UserList"] =
{
    Association: {
        ...
        load: function (inputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.IO.Input.User.List(inputId, index, size)
            });
        ...
        remove: function (inputId, userId) {
            var result = tryCatch(function () {
                return Client.Security().Permission.IO.Input.User.Delete(inputId, userId);
            })
            if (result)
    ...
    Unassociation: {
        ...
        load: function (inputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.IO.Input.User.List(inputId, index, size, true);
            });
            var result = new Object();
        ...
        add: function (inputId, userId, permission) {
            ...
            Client.Security().User.IO.Input.Create(userId, ioinputChannelPermission);
            delete this.value[userId];
...
Html.Control.PopoverWindow["UserList"] = {
    Common: {
        createItem: function (key, user) {
            var createCoontrol = {
                ...
                UnassociationOperationButton: function (user) {
                    ...
                    a.href = Property.Location.User.Details.Value + "?fun=PageEvent.Device.IO.Input.User.AddUserToInput&inputId=" + Html.Control.AlertWindow.Device.Input.Details.get().Id + "&userId=" + user.Id
                    a.setAttribute("onclick", "return GroupListItem_IOInputUnassociationAddUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.IO.Input.User.Unassociation.add + user.Id;
                    a.title = "添加";
                    return a;
                },
                AssociationOperationButton: function (user) {
                    ...
                    a.setAttribute("onclick", "GroupListItem_IOInputAssociationRemoveUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.IO.Input.User.Association.remove + user.Id;
                    a.title = "删除";
                    return a;
                },
            ...
            item = new GroupListItem(Info.ControlIdPrefix.Device.IO.Input.User[key].list, eval("IOInput" + key + "User_Selected"));
            item.id = Info.ControlIdPrefix.Device.IO.Input.User[key].list + user.Id;
        ...
        load: function (key, index) {
            ...
            if (items) {
                ...
                pagination = new Pagination("", page.PageIndex, page.PageSize, page.PageCount, page.RecordCount, page.TotalRecordCount, eval("IOInput" + key + "User_PageChange"));
    ...
    Association: {
        ...
        selectInverse: function () {
            Property.UserList.Association.load(Html.Control.AlertWindow.Device.Input.Details.get().Id);
            for (var uid in Property.UserList.Association.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.IO.Input.User.Association.list + uid);
        ...
        load: function (index) {
            ...
            PageEvent.Device.IO.Input.GroupListUserIconButtonChanged(Html.Control.AlertWindow.Device.Input.Details.get().Id, Property.UserList.Association.page.TotalRecordCount);
        },
        clear: function () {
    ...
    Unassociation: {
        ...
        selectInverse: function () {
            Property.UserList.Unassociation.load(Html.Control.AlertWindow.Device.Input.Details.get().Id);
            for (var uid in Property.UserList.Unassociation.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.IO.Input.User.Unassociation.list + uid);
...
PageEvent.Device.IO.Input.User.AddUserToInput = function (permission, userId) {
...
PageEvent.Device.IO.Input.User.BatchAddInputsToUser = function (permission) {
