...
Property["UserList"] =
{
    Association: {
        ...
        load: function (outputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.IO.Output.User.List(outputId, index, size)
            });
        ...
        remove: function (outputId, userId) {
            var result = tryCatch(function () {
                return Client.Security().Permission.IO.Output.User.Delete(outputId, userId);
            })
            if (result)
    ...
    Unassociation: {
        ...
        load: function (outputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.IO.Output.User.List(outputId, index, size, true);
            });
            var result = new Object();
        ...
        add: function (outputId, userId, permission) {
            var output = Client.Management().Device.IO.Output.Get(Property.DeviceId, outputId);
            var iooutputChannelPermission  = new IOOutputChannelPermission();
            iooutputChannelPermission.Id = output.Id;
            iooutputChannelPermission.Name = output.Name;
            iooutputChannelPermission.Permission = permission;
            //output.TriggeringTypeSpecified = true;
            iooutputChannelPermission.IOOutputChannel = output;            
            Client.Security().User.IO.Output.Create(userId, iooutputChannelPermission );
            delete this.value[userId];
...
Html.Control.PopoverWindow["UserList"] = {
    Common: {
        createItem: function (key, user) {
            var createCoontrol = {
                ...
                UnassociationOperationButton: function (user) {
                    ...
                    a.href = "output/user/" + Property.Location.User.Details.Value + "?fun=PageEvent.Device.IO.Output.User.AddUserToOutput&outputId=" + Html.Control.AlertWindow.Device.Output.Details.get().Id + "&userId=" + user.Id
                    a.setAttribute("onclick", "return GroupListItem_IOOutputUnassociationAddUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.IO.Output.User.Unassociation.add + user.Id;
                    a.title = "添加";
                    return a;
                },
                AssociationOperationButton: function (user) {
                    ...
                    a.setAttribute("onclick", "GroupListItem_IOOutputAssociationRemoveUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.IO.Output.User.Association.remove + user.Id;
                    a.title = "删除";
                    return a;
                },
            ...
            item = new GroupListItem(Info.ControlIdPrefix.Device.IO.Output.User[key].list, eval("IOOutput" + key + "User_Selected"));
            item.id = Info.ControlIdPrefix.Device.IO.Output.User[key].list + user.Id;
        ...
        load: function (key, index) {
            var result = Property.UserList[key].load(Html.Control.AlertWindow.Device.Output.Details.get().Id, index, Html.Control.PopoverWindow.PageSize);
            ...
            if (items) {
                ...
                pagination = new Pagination("", page.PageIndex, page.PageSize, page.PageCount, page.RecordCount, page.TotalRecordCount, eval("IOOutput" + key + "User_PageChange"));
    ...
    Association: {
        ...
        selectAll: function () {
            Property.UserList.Association.load(Html.Control.AlertWindow.Device.Output.Details.get().Id);
        ...
        selectInverse: function () {
            Property.UserList.Association.load(Html.Control.AlertWindow.Device.Output.Details.get().Id);
            for (var uid in Property.UserList.Association.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.IO.Output.User.Association.list + uid);
        ...
        load: function (index) {
            ...
            PageEvent.Device.IO.Output.GroupListUserIconButtonChanged(Html.Control.AlertWindow.Device.Output.Details.get().Id, Property.UserList.Association.page.TotalRecordCount);
        },
        clear: function () {
        ...
        remove: function (userId) {
            Property.UserList.Association.remove(Html.Control.AlertWindow.Device.Output.Details.get().Id, userId);
            this.clear();
        ...
        batchRemove: function () {
            ...
            for (var i = 0; i < this.selected.length; i++) {
                Property.UserList.Association.remove(Html.Control.AlertWindow.Device.Output.Details.get().Id, this.selected[i]);
                ++count;
            }
        ...
        removeAll: function () {
            ...
            for (var id in Property.UserList.Association.value) {
                count += Try(function () {
                    Property.UserList.Association.remove(outputId, id);
                    return 1;
                }, 0);
            }
            if (Property.UserList.Association.page.TotalRecordCount != count) {
                Property.UserList.Association.load(outputId, 1, Property.UserList.Association.page.TotalRecordCount);
                for (var id in Property.UserList.Association.value) {
                    count += Try(function () {
                        Property.UserList.Association.remove(outputId, id);
                        return 1;
    ...
    Unassociation: {
        ...
        selectAll: function () {
            Property.UserList.Unassociation.load(Html.Control.AlertWindow.Device.Output.Details.get().Id);
        ...
        selectInverse: function () {
            Property.UserList.Unassociation.load(Html.Control.AlertWindow.Device.Output.Details.get().Id);
            for (var uid in Property.UserList.Unassociation.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.IO.Output.User.Unassociation.list + uid);
        ...
        add: function (id, permission) {
            Property.UserList.Unassociation.add(Html.Control.AlertWindow.Device.Output.Details.get().Id, id, permission);
        ...
        batchAdd: function (permission) {
            for (var i = 0; i < this.selected.length; i++) {
                try {
                    Property.UserList.Unassociation.add(Html.Control.AlertWindow.Device.Output.Details.get().Id, this.selected[i], permission);
                } catch (e) {
...
PageEvent.Device.IO.Output.User.AddUserToOutput = function (permission, userId) {
...
PageEvent.Device.IO.Output.User.BatchAddOutputsToUser = function (permission) {
