...
Property["UserList"] =
{
    Association: {
        ...
        load: function (deviceId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.Device.User.List(deviceId, index, size)
            });
        ...
        remove: function (deviceId, userId) {
            var result = tryCatch(function () {
                return Client.Security().Permission.Device.User.Delete(deviceId, userId);
            })
            if (result)
    ...
    Unassociation: {
        ...
        load: function (deviceId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.Device.User.List(deviceId, index, size, true);
            });
            var result = new Object();
        ...
        add: function (deviceId, userId, permission) {
            var device = Client.Management().Device.Get(deviceId);
            var devicePermission = new DevicePermission();
            devicePermission.Id = device.Id;
            devicePermission.Name = device.Name;
            devicePermission.Permission = permission;
            devicePermission.Device = device;
            Client.Security().User.Device.Create(userId, devicePermission);
            delete this.value[userId];
...
Html.Control.PopoverWindow["UserList"] = {
    Common: {
        createItem: function (key, user) {
            var createCoontrol = {
                ...
                UnassociationOperationButton: function (user) {
                    ...
                    a.href = Info.ControlIdPrefix.User.AlertUrl.permissions + "?fun=PageEvent.Device.User.AddUserToDevice&deviceId=" + Html.Current.Id.get() + "&userId=" + user.Id
                    a.setAttribute("onclick", "return GroupListItem_DeviceUnassociationAddUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.User.Unassociation.add + user.Id
                    a.title = "添加";
                    return a;
                },
                AssociationOperationButton: function (user) {
                    ...
                    a.setAttribute("onclick", "GroupListItem_DeviceAssociationRemoveUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.User.Association.remove + user.Id;
                    a.title = "删除";
                    return a;
                },
            ...
            item = new GroupListItem(Info.ControlIdPrefix.User[key].list, eval("Device" + key + "User_Selected"));
            item.id = Info.ControlIdPrefix.User[key].list + user.Id;
        ...
        load: function (key, index) {
            var result = Property.UserList[key].load(Html.Current.Id.get(), index, Html.Control.PopoverWindow.PageSize);
            ...
            if (items) {
                ...
                pagination = new Pagination("", page.PageIndex, page.PageSize, page.PageCount, page.RecordCount, page.TotalRecordCount, eval("Device" + key + "User_PageChange"));
    ...
    Association: {
        ...
        selectAll: function () {
            Property.UserList.Association.load(Html.Current.Id.get());
        ...
        selectInverse: function () {
            Property.UserList.Association.load(Html.Current.Id.get());
            for (var uid in Property.UserList.Association.value) {
                var tag = getTag(Info.ControlIdPrefix.User.Association.list + uid);
        ...
        load: function (index) {
            ...
            PageEvent.Device.GroupListUserIconButtonChanged(Html.Current.Id.get(), Property.UserList.Association.page.TotalRecordCount);
        },
        clear: function () {
        ...
        remove: function (userId) {
            Property.UserList.Association.remove(Html.Current.Id.get(), userId);
            this.clear();
        ...
        batchRemove: function () {
            ...
            for (var i = 0; i < this.selected.length; i++) {
                Property.UserList.Association.remove(Html.Current.Id.get(), this.selected[i]);
                ++count;
            }
        ...
        removeAll: function () {
            ...
            for (var id in Property.UserList.Association.value) {
                count += Try(function () {
                    Property.UserList.Association.remove(deviceId, id);
                    return 1;
                }, 0);
            }
            if (Property.UserList.Association.page.TotalRecordCount != count) {
                Property.UserList.Association.load(deviceId, 1, Property.UserList.Association.page.TotalRecordCount);
                for (var id in Property.UserList.Association.value) {
                    count += Try(function () {
                        Property.UserList.Association.remove(deviceId, id);
                        return 1;
    ...
    Unassociation: {
        ...
        selectAll: function () {
            Property.UserList.Unassociation.load(Html.Current.Id.get());
        ...
        selectInverse: function () {
            Property.UserList.Unassociation.load(Html.Current.Id.get());
            for (var uid in Property.UserList.Unassociation.value) {
                var tag = getTag(Info.ControlIdPrefix.User.Unassociation.list + uid);
        ...
        add: function (id, permission) {
            Property.UserList.Unassociation.add(Html.Current.Id.get(), id, permission);

            if (Property.UserList.Unassociation.page.RecordCount == 1 && Property.UserList.Unassociation.page.PageIndex == Property.UserList.Unassociation.page.PageCount && Property.UserList.Unassociation.page.PageIndex != 1)
        ...
        batchAdd: function (permission) {
            for (var i = 0; i < this.selected.length; i++) {
                try {
                    Property.UserList.Unassociation.add(Html.Current.Id.get(), this.selected[i], permission);
                } catch (e) {
...
PageEvent.Device.User.AddUserToDevice = function (permission, userId) {
...
PageEvent.Device.User.BatchAddUsersToDevice = function (permission) {
