...
Property["UserList"] =
{
    Association: {
        ...
        load: function (outputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.Video.Output.User.List(outputId, index, size)
            });
        ...
        remove: function (outputId, userId) {
            var result = tryCatch(function () {
                return Client.Security().Permission.Video.Output.User.Delete(outputId, userId);
            })
            if (result)
    ...
    Unassociation: {
        ...
        load: function (outputId, index, size) {
            var response = tryCatch(function () {
                return Client.Security().Permission.Video.Output.User.List(outputId, index, size, true);
            });
            var result = new Object();
        ...
        add: function (outputId, userId, permission) {
            ...
            Client.Security().User.Video.Output.Create(userId, videoOutputChannelPermission);
            delete this.value[userId];
...
Html.Control.PopoverWindow["UserList"] = {
    Common: {
        createItem: function (key, user) {
            var createCoontrol = {
                ...
                UnassociationOperationButton: function (user) {
                    ...
                    a.href = "output/user/" + Property.Location.User.Details.Value + "?fun=PageEvent.Device.Video.Output.User.AddUserToOutput&outputId=" + Html.Control.AlertWindow.Device.Output.Details.get().Id + "&userId=" + user.Id
                    a.setAttribute("onclick", "return GroupListItem_OutputUnassociationAddUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.Video.Output.User.Unassociation.add + user.Id;
                    a.title = "添加";
                    return a;
                },
                AssociationOperationButton: function (user) {
                    ...
                    a.setAttribute("onclick", "GroupListItem_OutputAssociationRemoveUser_Click(this, '" + user.Id + "')");
                    a.id = Info.ControlIdPrefix.Device.Video.Output.User.Association.remove + user.Id;
                    a.title = "删除";
                    return a;
                },
            ...
            item = new GroupListItem(Info.ControlIdPrefix.Device.Video.Output.User[key].list, eval("Output" + key + "User_Selected"));
            item.id = Info.ControlIdPrefix.Device.Video.Output.User[key].list + user.Id;
        ...
        load: function (key, index) {
            ...
            if (items) {
                ...
                pagination = new Pagination("", page.PageIndex, page.PageSize, page.PageCount, page.RecordCount, page.TotalRecordCount, eval("Output" + key + "User_PageChange"));
    ...
    Association: {
        ...
        selectInverse: function () {
            Property.UserList.Association.load(Html.Control.AlertWindow.Device.Output.Details.get().Id);
            for (var uid in Property.UserList.Association.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.Video.Output.User.Association.list + uid);
        ...
        load: function (index) {
            ...
            PageEvent.Device.Video.Output.GroupListUserIconButtonChanged(Html.Control.AlertWindow.Device.Output.Details.get().Id, Property.UserList.Association.page.TotalRecordCount);
        },
        clear: function () {
    ...
    Unassociation: {
        ...
        selectInverse: function () {
            Property.UserList.Unassociation.load(Html.Control.AlertWindow.Device.Output.Details.get().Id);
            for (var uid in Property.UserList.Unassociation.value) {
                var tag = getTag(Info.ControlIdPrefix.Device.Video.Output.User.Unassociation.list + uid);
...
PageEvent.Device.Video.Output.User.AddUserToOutput = function (permission, userId) {
...
PageEvent.Device.Video.Output.User.BatchAddOutputsToUser = function (permission) {
