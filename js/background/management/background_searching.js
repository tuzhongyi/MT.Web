...
var Property =
{
    ...
    Searching:
    {
        ...
        get: function () {
            ...
            try {
                device = Client.Management().Device.Searching.Get(Property.Guid);
            }
            catch (e) {
                switch (e.number) {
                    case 404:
                ...
                if (this.value[id].Selected && this.value[id].ExistedInDatabase == false) {
                    tryCatch(function () {
                        ...
                        if (userName && password) {
                            Property.Searching.value[id].Username = userName;
                            Property.Searching.value[id].Password = password;
                        }
    ...
    Control: {
        Table: {
            classificationColor: {
                ...
                RFIDAntenna: LabelTagColor.Taro,
                Detector: LabelTagColor.Color_66cc99,
                ExtensionUnit: LabelTagColor.Yellow,
                PDCCamera: LabelTagColor.LightBlue,
                RadarCamera: LabelTagColor.LightBlue,
                ParkingDetector: LabelTagColor.Color_66cc99,
                GuideScreen: LabelTagColor.DoderBlue,
                FDCamera: LabelTagColor.LightBlue,
                FDNVR: LabelTagColor.Red
            },
            createItem: function (device) {
                ...
                tr.appendChild(new TableBodyTd("<div title=" + Language.Enum.DeviceClassification[device.Classification] + " class='icon-color-btn-background " + this.classificationColor[device.Classification] + "' style='margin-right: 10px;'><div class='arrow-left icon-color-btn-triangle'></div><div class='background_icon classification device " + device.Classification.toLowerCase() + "'></div></div>", "td-classification"));
            ...
            start: function (protocolType) {
                ...
                Property.Timer = setInterval(function () {
                    ...
                    if (device) {
                        Html.Control.Table.load(device);
                    }
                }, 500);
            },
            stop: function () {
                clearInterval(Property.Timer);
                Property.Searching.stop();
                if (Property.Searching.type && Property.Searching.type.length > 0) {
                    ...
                    for (var i = 0; i < Property.Searching.type.length; i++) {
                        ...
                        li.appendChild($("<a class='device-type " + Property.Searching.type[i].toLowerCase() + " xsgroupdown-menu-ul-li-a' onclick=\"Html.Control.Table.selectType(this,'" + Property.Searching.type[i] + "')\">" + Property.Searching.type[i] + "-" + Language.Enum.DeviceClassification[Property.Searching.type[i]] + "</a>")[0]);
                        var headLi = document.createElement("li");
                        headLi.appendChild($("<a class='device-type " + Property.Searching.type[i].toLowerCase() + " xsgroupdown-menu-ul-li-a' onclick=\"Html.Control.Table.selectType(this,'" + Property.Searching.type[i] + "')\">" + Property.Searching.type[i] + "-" + Language.Enum.DeviceClassification[Property.Searching.type[i]] + "</a>")[0]);
                        list.appendChild(li);
