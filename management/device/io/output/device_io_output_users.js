...
//用户选择事件
function IOOutputAssociationUser_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.IO.Output.User.Association.list);
...
//删除单个用户
function GroupListItem_IOOutputAssociationRemoveUser_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.IO.Output.User.Association.remove);
        $.confirm({
            text: "确认要删除这个用户吗？",
            okButton: "确定",
            cancelButton: "取消",
...
//已绑定用户页面改变事件
function IOOutputAssociationUser_PageChange(index) {
    userList.Association.clear();
...
//已绑定的用户当前页全部选中事件
function IOOutputAssociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Output.User.Association.list);
...
//已绑定的用户当前页全部选择取消事件
function IOOutputAssociationUser_SelectedCancel(items) {
    userList.Association.selected = new Array();
}
function IOOutputAssociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Output.User.Association.list);
        if (userList.Association.selected.indexOf(id) < 0)
            userList.Association.selected.push(id);
...
function IOOutputAssociationUser_SelectedInverse() {
...
function btn_user_association_Click(sender, args) {
    ...
    //userList.Association.show();
    if (userList.Unassociation.selected.length > 0)
        PageEvent.Device.IO.Output.User.BatchAddOutputsToUser(IOOutputPermissions.All);
    else
...
//未绑定用户绑定事件
function GroupListItem_IOOutputUnassociationAddUser_Click(sender, args) {
    //stopPropagation();
    //tryCatch(function () {
    ...
    //});
    stopPropagation();
    PageEvent.Device.IO.Output.User.AddUserToOutput(IOOutputPermissions.All, args);
    //AlertWindow.Show(sender, -1);
    return false;
}
//未绑定用户选择事件
function IOOutputUnassociationUser_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.IO.Output.User.Unassociation.list);
        userList.Unassociation.select(id);
    });
}

function IOOutputUnassociationUser_PageChange(index) {
    userList.Unassociation.clear();
    userList.Unassociation.load(index);
}
function IOOutputUnassociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Output.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
    }
    userList.Unassociation.selectAll();
}
function IOOutputUnassociationUser_SelectedCancel(items) {
    userList.Unassociation.selected = new Array();
}
function IOOutputUnassociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Output.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
...
function IOOutputUnassociationUser_SelectedInverse() {
