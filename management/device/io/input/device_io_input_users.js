...
//用户选择事件
function IOInputAssociationUser_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.IO.Input.User.Association.list);
...
//删除单个用户
function GroupListItem_IOInputAssociationRemoveUser_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.IO.Input.User.Association.remove);
        $.confirm({
            text: "确认要删除这个用户吗？",
            okButton: "确定",
            cancelButton: "取消",
...
//已绑定用户页面改变事件
function IOInputAssociationUser_PageChange(index) {
    userList.Association.clear();
...
//已绑定的用户当前页全部选中事件
function IOInputAssociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Input.User.Association.list);
...
//已绑定的用户当前页全部选择取消事件
function IOInputAssociationUser_SelectedCancel(items) {
    userList.Association.selected = new Array();
}
function IOInputAssociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Input.User.Association.list);
        if (userList.Association.selected.indexOf(id) < 0)
            userList.Association.selected.push(id);
...
function IOInputAssociationUser_SelectedInverse() {
...
//未绑定用户绑定事件
function GroupListItem_IOInputUnassociationAddUser_Click(sender, args) {
    //stopPropagation();
    //tryCatch(function () {
...
//未绑定用户选择事件
function IOInputUnassociationUser_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.IO.Input.User.Unassociation.list);
        userList.Unassociation.select(id);
    });
}

function IOInputUnassociationUser_PageChange(index) {
    userList.Unassociation.clear();
    userList.Unassociation.load(index);
}
function IOInputUnassociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Input.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
    }
    userList.Unassociation.selectAll();
}
function IOInputUnassociationUser_SelectedCancel(items) {
    userList.Unassociation.selected = new Array();
}
function IOInputUnassociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.IO.Input.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
...
function IOInputUnassociationUser_SelectedInverse() {
