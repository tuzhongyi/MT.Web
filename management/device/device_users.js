...
//用户选择事件
function DeviceAssociationUser_Selected(sender, args) {
    tryCatch(function () {
...
//删除单个用户
function GroupListItem_DeviceAssociationRemoveUser_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
//已绑定用户页面改变事件
function DeviceAssociationUser_PageChange(index) {
    userList.Association.clear();
...
//已绑定的用户当前页全部选中事件
function DeviceAssociationUser_SelectedAll(items) {
...
//已绑定的用户当前页全部选择取消事件
function DeviceAssociationUser_SelectedCancel(items) {
    userList.Association.selected = new Array();
}
function DeviceAssociationUser_SelectedInverse() {
    userList.Association.selectInverse();
}
function DeviceAssociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.User.Association.list);
        if (userList.Association.selected.indexOf(id) < 0)
            userList.Association.selected.push(id);
...
//未绑定用户绑定事件
function GroupListItem_DeviceUnassociationAddUser_Click(sender, args) {
    //stopPropagation();
    //tryCatch(function () {
...
//未绑定用户选择事件
function DeviceUnassociationUser_Selected(sender, args) {
    tryCatch(function () {
...
function DeviceUnassociationUser_PageChange(index) {
    userList.Unassociation.clear();
    userList.Unassociation.load(index);
}
function DeviceUnassociationUser_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
...
function DeviceUnassociationUser_SelectedCancel(items) {
    userList.Unassociation.selected = new Array();
}

function DeviceUnassociationUser_SelectedInverse() {
    userList.Unassociation.selectInverse();
}
function DeviceUnassociationUser_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.User.Unassociation.list);
        if (userList.Unassociation.selected.indexOf(id) < 0)
            userList.Unassociation.selected.push(id);
