...
//部门选择事件
function InputAssociationDepartment_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.Video.Input.Department.Association.list);
...
//删除单个部门
function GroupListItem_InputAssociationRemoveDepartment_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.Video.Input.Department.Association.remove);
        $.confirm({
            text: "确认要删除这个部门吗？",
            okButton: "确定",
            cancelButton: "取消",
...
//已绑定部门页面改变事件
function InputAssociationDepartment_PageChange(index) {
    departmentList.Association.clear();
...
//已绑定的部门当前页全部选中事件
function InputAssociationDepartment_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.Video.Input.Department.Association.list);
...
//已绑定的部门当前页全部选择取消事件
function InputAssociationDepartment_SelectedCancel(items) {
    departmentList.Association.selected = new Array();
}
function InputAssociationDepartment_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.Video.Input.Department.Association.list);
        if (departmentList.Association.selected.indexOf(id) < 0)
            departmentList.Association.selected.push(id);
...
function InputAssociationDepartment_SelectedInverse() {
...
//未绑定部门绑定事件
function GroupListItem_InputUnassociationAddDepartment_Click(sender, args) {
    //stopPropagation();
    //tryCatch(function () {
...
//未绑定部门选择事件
function InputUnassociationDepartment_Selected(sender, args) {
    tryCatch(function () {
        var id = Info.ControlIdPrefix.getId(sender.id, Info.ControlIdPrefix.Device.Video.Input.Department.Unassociation.list);
        departmentList.Unassociation.select(id);
    });
}

function InputUnassociationDepartment_PageChange(index) {
    departmentList.Unassociation.clear();
    departmentList.Unassociation.load(index);
}
function InputUnassociationDepartment_SelectedAll(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.Video.Input.Department.Unassociation.list);
...
function InputUnassociationDepartment_SelectedCancel(items) {
    departmentList.Unassociation.selected = new Array();
}
function InputUnassociationDepartment_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Device.Video.Input.Department.Unassociation.list);
        if (departmentList.Unassociation.selected.indexOf(id) < 0)
            departmentList.Unassociation.selected.push(id);
...
function InputUnassociationDepartment_SelectedInverse() {
