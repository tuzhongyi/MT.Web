...
function device_video_inputs_load() {
    document.title = "视频输入通道列表";
    ...
    //$($.parseHTML(loadPage("device_video_inputs.js.htm"), document, true)).insertAfter(getTag("sc"));
    ...
    if (url.Querys && url.Querys.deviceName) {
        var win = new AlertWindow("", "<a class='btn btn-xs btn-primary-outline pull-right div-xsgroup' style='font-size:14px;padding-left:14px;'><i class='icon-plus'></i></a>", "management/device/video/input/device_video_input_add.htm?deviceName=" + url.Querys.deviceName, -1);
...
function batchCompareVideoInputs_Click(sender, args) {
    if (Html.Control.GroupList.Count.Selected > 0) {
        Property.Input.batchCompare();
        window.location.href = "diagnostics/device/video/input/list.htm";
    }
}

function batchRemoveVideoInputs_Click(sender, args) {
    if (Html.Control.GroupList.Count.Selected > 0) {
        $.confirm({
            text: "确定要删除所有选中的通道吗？",
            okButton: "确定",
...
PageEvent.Device.Video.Input.GroupListItemRemoveButtonClick = function (sender, args) {
