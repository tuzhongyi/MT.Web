...
function btnDetailDevice_Click(sender, args) {
    if (Html.Control.AlertWindow.IsBatch.get()) {
        ...
        if (Html.Control.AlertWindow.IsModify.get()) {
            ...
            if (hasCapabilitiesDevice[device.Classification]) {
                ...
                device.ProtocolType = getTag("txtProtocolType").value;
                device.Uri = "tcp://" + ipControl.toString() + ":" + getTag("txtPort").value + "/";
            }
...
function detailClassification_Onchange(sender, args) {
    var protocolType = Property.Device.capabilities(sender.value);
    getTag("ddlProtocolType").innerHTML = "";
    if (protocolType && protocolType.DeviceProtocolCapabilitiesSpecified) {
        Html.Control.AlertWindow.deviceProtocolCapabilities = new Object();
        var displayName = new Object();
        var displayLanguage = new Object();

        for (var i = 0; i < protocolType.DeviceProtocolCapabilities.length; i++) {
            Html.Control.AlertWindow.deviceProtocolCapabilities[protocolType.DeviceProtocolCapabilities[i].ProtocolType] = protocolType.DeviceProtocolCapabilities[i];
            displayName[protocolType.DeviceProtocolCapabilities[i].ProtocolType] = protocolType.DeviceProtocolCapabilities[i].ProtocolType;
            displayLanguage[protocolType.DeviceProtocolCapabilities[i].ProtocolType] = protocolType.DeviceProtocolCapabilities[i].Comment;
            if (!displayLanguage[protocolType.DeviceProtocolCapabilities[i].ProtocolType])
                displayLanguage[protocolType.DeviceProtocolCapabilities[i].ProtocolType] = protocolType.DeviceProtocolCapabilities[i].ProtocolType;
        }
        DropDownList.Create("ddlProtocolType", ControlModel.Language, displayName, displayLanguage);
    }
}