...
//部门选择事件
function DeviceAssociationDepartment_Selected(sender, args) {
    tryCatch(function () {
...
//删除单个部门
function GroupListItem_DeviceAssociationRemoveDepartment_Click(sender, args) {
    stopPropagation();
    tryCatch(function () {
...
//已绑定部门页面改变事件
function DeviceAssociationDepartment_PageChange(index) {
    departmentList.Association.clear();
...
//已绑定的部门当前页全部选中事件
function DeviceAssociationDepartment_SelectedAll(items) {
...
//已绑定的部门当前页全部选择取消事件
function DeviceAssociationDepartment_SelectedCancel(items) {
    departmentList.Association.selected = new Array();
}
function DeviceAssociationDepartment_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Department.Association.list);
        if (departmentList.Association.selected.indexOf(id) < 0)
            departmentList.Association.selected.push(id);
...
function DeviceAssociationDepartment_SelectedInverse() {
...
//未绑定部门绑定事件
function GroupListItem_DeviceUnassociationAddDepartment_Click(sender, args) {
    //stopPropagation();
    //tryCatch(function () {
...
//未绑定部门选择事件
function DeviceUnassociationDepartment_Selected(sender, args) {
    tryCatch(function () {
...
function DeviceUnassociationDepartment_PageChange(index) {
    departmentList.Unassociation.clear();
    departmentList.Unassociation.load(index);
}
function DeviceUnassociationDepartment_SelectedAll(items) {
      for (var i = 0; i < items.length; i++) {
...
function DeviceUnassociationDepartment_SelectedCancel(items) {
    departmentList.Unassociation.selected = new Array();
}
function DeviceUnassociationDepartment_SelectedCurrent(items) {
    for (var i = 0; i < items.length; i++) {
        var id = Info.ControlIdPrefix.getId(items[i].id, Info.ControlIdPrefix.Department.Unassociation.list);
        if (departmentList.Unassociation.selected.indexOf(id) < 0)
            departmentList.Unassociation.selected.push(id);
...
function DeviceUnassociationDepartment_SelectedInverse() {
